...
from .datastore import load_compromised_sites, sanitize_return, polling_load_compromised_sites
...
def polling_get_all_breaches_for_user(user_email):
    return polling_build_detailed_breach_information(user_email, breach_service.getAllBreachesForAccount(email=user_email))
...
def polling_build_detailed_breach_information(user_email, breach_list):

    # This needs to be run by acknowledged vs unacknowledged list in firebase

    polling_load_compromised_sites(user_email=user_email, detailed_breach_info=breach_list)
